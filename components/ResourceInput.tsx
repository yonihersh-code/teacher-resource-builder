...
const ResourceInput: React.FC<ResourceInputProps> = ({ prompt, setPrompt, handleGenerate, isLoading }) => {
  return (
    <div className="bg-white dark:bg-slate-800/50 p-6 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 w-full">
      ...
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
        Be as descriptive as possible. Include the topic, grade level, and type of resource (e.g., "a 5th-grade worksheet on the water cycle with 10 fill-in-the-blank questions").
      </p>
      <textarea
        id="resource-prompt"
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        placeholder="e.g., A high school biology lesson plan on cellular respiration..."
        className="w-full h-48 p-4 border border-slate-300 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-blue-500 focus:outline-none transition duration-200 resize-none"
        disabled={isLoading}
      />
      <button
